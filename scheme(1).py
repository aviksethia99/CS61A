...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        #return .apply_macro(first.formals, env)
    else:
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        procedure = scheme_eval(first, env)
        ...
        if isinstance(procedure, MacroProcedure):
            callexpr = procedure.apply_macro(rest, env)
            return scheme_eval(callexpr, env)
        arguments = rest.map(lambda val: scheme_eval(val, env))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        lambdaproc = Pair('lambda', Pair(target.second, expressions.second))
        env.define(target.first, scheme_eval(lambdaproc, env, True))
        return target.first
        # END PROBLEM 10
    else:
...
def do_define_macro(expressions, env):
    ...
    target = expressions.first
    #if scheme_symbolp(target):
        #env.define(target, scheme_eval(expressions.second.first, env, True))
        #return target.first
