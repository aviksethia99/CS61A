...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('its no bueno friendo')
        else:
            def recursive_helper(f, v):
                if not f == nil and not v == nil:
                    child.define(f.first,v.first)
                    recursive_helper(f.second, v.second)
            recursive_helper(formals, vals)
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        # if env.parent != None:
        #     return env.parent.make_child_frame(self.formals,args)
...
def do_and_form(expressions, env):
    ...
    eval=scheme_eval(expressions.first, env)
    if scheme_falsep(eval):
        return False
...
def do_or_form(expressions, env):
    ...
    if expressions == (nil):
        ...
        while expressions != (nil):
            ...
            final = scheme_eval(expressions.first, env,True)
            if scheme_truep(final):
                return final
...
def make_let_frame(bindings, env):
    ...
    if len(bindings.first) != 2:
        raise SchemeError('2')
    if bindings.first.first == bindings.first.second.first:
        raise SchemeError('1')
    formal = Pair(bindings.first.first, nil)
    values = Pair(scheme_eval(bindings.first.second.first,env), nil)
    ...
    return env.make_child_frame(formal, values)
...
def do_define_macro(expressions, env):
    ...
    # BEGIN Problem 21
    target = expressions.first
    if isinstance(target, Pair) and scheme_symbolp(target.first) and expressions.second:
        macroproc = MacroProcedure(target.second, expressions.second, env)
        env.define(target.first, macroproc)
        return target.first
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
