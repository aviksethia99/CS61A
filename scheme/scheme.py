...
def do_or_form(expressions, env):
    ...
    if expressions == (nil):
        ...
        while expressions != (nil):
            ...
            final = scheme_eval(expressions.first, env)
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(expressions.first, eval_all(expressions.second, env))

        return expressions.first

    elif isinstance(target, Pair) and scheme_symbolp(target.first):

        mcr = MacroProcedure(target.second, expressions.second, env)

        env.define(target.first, mcr)
