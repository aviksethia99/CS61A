...
(define fib 'fib)
; expect fib
(define (quadratic-determinant coa cob coc) (- (* cob cob) (* 4 coa coc)))
; expect quadratic-determinant
(quadratic-determinant 5 3 10)
;expect -191
(define (square-root num) (sqrt num))
; expect square-root
(square-root 25)
; expect 5.0
(define (quadratic-formula coa cob coc)
  (cons
    (/
      (- (- cob) (square-root (quadratic-determinant coa cob coc)))
        (* 2 coa))
          (cons
    (/
      (+ (- cob) (square-root (quadratic-determinant coa cob coc)))
        (* 2 coa))
         nil)))
; expect quadratic-formula

(quadratic-formula 1 5 6)
; expect (-3 -2)
(define (is-imaginary-root? coa cob coc) (if (< (quadratic-determinant coa cob coc) 0) #t #f))
; expect is-imaginary-root?
(define (is-real-root? coa cob coc) (if (is-imaginary-root? coa cob coc) #f #t))
; expect is-real-root?
(define (what-type-of-root? coa cob coc) (cond ((is-imaginary-root? coa cob coc) 'root-is-imaginary)
  ((is-real-root? coa cob coc) 'root-is-real)
    (else 'root-is-0)
  ))

; expect what-type-of-root?

(is-imaginary-root? 1 0 1)
; expect #t
(is-real-root? 1 0 (- 1))
; expect #t
(what-type-of-root? 1 7 3)
; expect root-is-real
(what-type-of-root? 1 2 3)
; expect root-is-imaginary

(define (imaginary? girlfriend)
  (define (take-money from-me) (- 100 (* girlfriend from-me)))
  (take-money 2))
; expect imaginary?
